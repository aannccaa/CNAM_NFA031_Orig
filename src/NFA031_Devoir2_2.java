/*
a) Ecrire un programme qui demande à un utilisateur de donner N caractères qui sont soit des chiffres, soit des lettres, N étant un entier fourni par l'utilisateur au début de l'exécution du programme. Ces caractères seront stockés dans un tableau 'tab'. 
...
N.B: vous pouvez utiliser la méthode: Character.isDigit, qui prend en argument un caractère et retourne ‘true’ 
si le caractère est un nombre et ‘false’ dans le cas contraire, 
...
public class NFA031_Devoir2_2 {
	...
	// concatenner 2 tableaux, en copiant un a un tous les éléments du tab1, puis du tab2, 
	//dans la première position libre (qui sera incrémentée après chaque copie)
	...
	// concatenner 2 tableaux (variante), en copiant à partir de la position 0 les valeurs du tableau tab1, 
	...
	public static char[] creerTabbis(char[] tab, int nombreChiffres, int nombreLettres) {
		...
		// parcourir tab et remplir d'un seul passage les 2 tableaux de chiffres et lettres
		...
		if (nombreChiffres > nombreLettres) {
			// le resultat est la concatenation du tableau de chiffres avec celui de lettres
			tabbis = concatTableaux(tabChiffres, tabLettres);
		} else {
			// le resultat est la concatenation du tableau de lettres avec celui de lettres
	...
	// fonction pour créer le tableau tabbis (variante)
	public static char[] creerTabbis1(char[] tab, int nombreChiffres, int nombreLettres) {
		...
		if (nombreChiffres > nombreLettres) {
			int positionLibre = 0;
			// parcourir le tableau jusqu'un ciffre est trouvé. L'ajouter dans tabbis sur la plus petite position libre.
			for (int i = 0; i < tab.length; i++) {
				char temp = tab[i];
				if (estChiffre(temp)) {
			...
			// parcourir le tableau jusqu'une lettre est trouvée. L'ajouter dans tabbis sur la plus petite position libre 
			for (int i = 0; i < tab.length; i++) {
				char temp = tab[i];
				if (estLettre(temp)) {
					tabbis[positionLibre] = temp;
